...
/**
 * PHASE 1 — LoanForm
 * Fixed payload to match backend ApplyLoanDto exactly:
 *   clientId (number), bikeId (number|undefined), amount, months, interestRate
 * Removed: client_id, loan_type, principal_amount, term_period, start_date
 *   (those were fields the old broken payload sent that didn't match the DTO)
 */
export function LoanForm({ clientId, onSuccess }: { clientId?: number; onSuccess: () => void }) {
  ...
  const handlePost = async (e: React.FormEvent) => {
    ...
    const resolvedClientId: number | undefined =
      selectedClient?.id ?? (typeof clientId === 'number' ? clientId : undefined);

    if (!resolvedClientId) {
      return toast.error('Please select a client before submitting.');
    }
    ...
    try {
      ...
      const payload: Record<string, any> = {
        ...
        loanType:     loanType.toLowerCase() as 'cash' | 'bike',
        notes:        `[POLICY 2026-01-10]: ${notes}`,
      };

      if (loanType === 'BIKE' && selectedBike?.id) {
        payload.bikeId = selectedBike.id;
      }
    ...
    <form onSubmit={handlePost} className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-1">
      <div className="lg:col-span-7 bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-6">
        <div className="flex bg-slate-100 p-1 rounded-2xl w-fit">
          <button type="button" onClick={() => setLoanType('CASH')}
            className={`px-8 py-2.5 rounded-xl font-black text-xs transition-all ${loanType === 'CASH' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400'}`}>
            CASH
          </button>
          <button type="button" onClick={() => setLoanType('BIKE')}
            className={`px-8 py-2.5 rounded-xl font-black text-xs transition-all ${loanType === 'BIKE' ? 'bg-white text-orange-600 shadow-sm' : 'text-slate-400'}`}>
            BIKE
          </button>
        </div>
        ...
        <div className="space-y-4">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
            {loanType === 'CASH' ? 'Principal (UGX)' : 'Bike Sale Price'}
          </label>

          {loanType === 'BIKE' && (
            <BikeSearch onSelect={(bike) => {
        ...
        <div className="pt-6 border-t border-red-50">
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)}
            className="w-full p-5 bg-red-50/10 border border-red-100 rounded-2xl text-sm h-32 outline-none"
            placeholder="Explain the purpose of this disbursement..." />
        </div>
      </div>
      ...
      <div className="lg:col-span-5 flex flex-col gap-6">
        <div className={`p-10 rounded-[3rem] text-white shadow-xl flex-1 flex flex-col justify-between ${loanType === 'CASH' ? 'bg-blue-600' : 'bg-orange-600'}`}>
          ...
          <div>
            ...
            <h2 className="text-5xl font-black mt-2 tracking-tighter">
              {results.installment.toLocaleString()} <span className="text-sm opacity-50 uppercase font-mono">UGX</span>
            </h2>
        ...
        <button type="submit" disabled={loading}
          className="w-full py-8 bg-slate-900 text-white rounded-[2.5rem] font-black text-xl hover:bg-black transition-all flex items-center justify-center gap-4 disabled:opacity-50 shadow-2xl">
        ...
        <div className="flex items-center gap-3 p-4 bg-slate-100 rounded-2xl border border-slate-200">
          ...
          <p className="text-[9px] leading-tight text-slate-500 font-bold uppercase font-mono italic">
            All loans require admin approval before activation [Policy 2026-01-10].
          </p>
        </div>
      </div>
    </form>
